...
describe("Delete book service unit tests", async () => {
  test("Delete book service if have an argument should return a new created book", async () => {
    const service = new DeleteBookService();
    const result = await service.execute(mock);
    expect(result).toHaveProperty("id");
  });
  test("Delete book service if try to delete a nonexistent book, it will fail", async () => {
    try {
      const service = new DeleteBookService();
